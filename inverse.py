...
n_sample = len(reco_validation)#1000

for i in range(n_sample):

	if i%1000:
		print("inversing ", i, "th event")
	...
	#electron
	reco_e = reco_validation[i:i+1, [0,1,2]]
	mean_e = means[[0,1,2]]
	std_e = stds[[0,1,2]]
	#proton
	reco_p = reco_validation[i:i+1, [3,4,5]]
	mean_p = means[[3,4,5]]
	std_p = stds[[3,4,5]]
	#photon
	reco_g = reco_validation[i:i+1, [6,7,8]]
	mean_g = means[[6,7,8]]
	std_g = stds[[6,7,8]]
...
df_Truths.to_pickle("gendata/Cond/3features/UMNN/Truths_dvcs.pkl")#.format(num_features,
